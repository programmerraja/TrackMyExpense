...
export const useBankStatement = (statementId = null) => {
  ...
  const updateStatementStatus = useCallback((status, errorMessage = null) => {
    if (bankStatement) {
  ...
  const updateTransactionNarration = useCallback((transactionId, narration) => {
  ...
  const getFilteredTransactions = useCallback(() => {
    return transactions.filter(txn => {
      // Date range filter
      if (filters.dateRange.start && txn.transactionDate < filters.dateRange.start) {
        return false;
      }
      if (filters.dateRange.end && txn.transactionDate > filters.dateRange.end) {
        return false;
      }
      
      // Search text filter
      if (filters.searchText && !txn.narration.toLowerCase().includes(filters.searchText.toLowerCase())) {
        return false;
      }
  ...
  const getSummaryData = useCallback(() => {
    ...
    const totalIncome = filteredTxs.reduce((sum, txn) => sum + txn.creditAmount, 0);
    const totalExpenses = filteredTxs.reduce((sum, txn) => sum + txn.debitAmount, 0);
